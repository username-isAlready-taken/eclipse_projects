...
public class MenuState extends State {
	...
	public void update() {
		if(Game.getHandler().getKeyManager().getKeyPressedOnce(KeyEvent.VK_UP)) {
			...
			if(item == MainMenuItems.PLAY.getValue()) {
				Game.getHandler().getGameStateManager().switchState(GameStates.PLAYSTATE);
			}
	...
	public void render() {
		...
		x = Game.getHandler().getDisplay().getWidth()/2 - g.getFontMetrics().stringWidth(menuItems.get(0));
		
		for(String s: menuItems) {
			if(menuItems.indexOf(s) == selection) {
				g.setColor(Color.WHITE);
			}
			g.drawString(s, x, 400 + 30*(menuItems.indexOf(s)+1));
			g.setColor(Color.GREEN);
		}
		
		g.drawRect(x-20, 400, 110, 20+menuItems.size()*30);
